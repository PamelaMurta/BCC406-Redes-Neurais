#!/usr/bin/env python
"""
Train Random Forest Model for Speaker Identification

This script trains a Random Forest classifier on aggregated acoustic features.
"""
...
from src.evaluation.visualization import (
    plot_confusion_matrix, plot_per_speaker_accuracy, plot_feature_importance
)
...
def main():
    parser = argparse.ArgumentParser(
        description='Train Random Forest model for speaker identification'
    )
    parser.add_argument(
        '--config',
        type=str,
    ...
    parser.add_argument(
        '--model-path',
        type=str,
        default='models/random_forest_best.pkl',
    ...
    logger.info("=" * 60)
    logger.info("Random Forest Training Script")
    logger.info("=" * 60)
    ...
    # Set random seeds
    seed = args.seed if args.seed is not None else config.get('seeds', {}).get('sklearn', 42)
    ...
    fig = plot_per_speaker_accuracy(
        ...
        title='Random Forest - Per-Speaker Accuracy (Validation)',
